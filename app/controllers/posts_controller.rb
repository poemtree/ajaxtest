class PostsController < ApplicationController
  ...
  def map_data
    ...
    indices = JSON.parse(params[:indices])
    schools = School.where("(lat BETWEEN ? and ?) and (lng BETWEEN ? and ?)", min["_lat"],max["_lat"], min["_lng"], max["_lng"])
    school_id = schools.map {|x| x.id }
    school_id -= indices
    # @school = School.where("(lat BETWEEN ? and ?) and (lng BETWEEN ? and ?)", min["_lat"],max["_lat"], min["_lng"], max["_lng"])
    # @school = School.all.limit(2000)
